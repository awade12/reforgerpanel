...
import { InstancePreflightPanel, localPreflight } from "@/components/instance-preflight-panel";
...
function localDiagnostics(config: ServerConfig): Diagnostics {
  return {
    preflight: localPreflight(config),
...
export function InstanceNetworkPanel({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setLoadError(
        message === "Not found"
          ? "Host agent is running old code — restart reforgerpanel-agent after git pull."
          : message,
      );
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <InstancePreflightPanel
        preflight={diagnostics.preflight}
        loading={loading}
        onRefresh={() => void refresh()}
        onForceStart={onForceStart}
        showForceStart={status !== "running" && status !== "starting"}
      />
        ...
        <div className="grid gap-4 p-4 lg:grid-cols-2">
          <div className="border border-border p-3">
            ...
            <p className="mt-2 text-[11px] text-muted-foreground">
              Forward these on your router and provider firewall (OVH cloud firewall). Game port is required for players;
              A2S is for server browser queries.
            </p>
          ...
          <div className="border border-border p-3">
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs font-medium text-foreground">Registration summary</p>
            ...
            {diagnostics.a2s && (
              <p className="mt-2 text-xs text-muted-foreground">
                A2S:{" "}
                {diagnostics.a2s.listed
                  ? `listed · ${diagnostics.a2s.playerCount}`
                  : diagnostics.a2s.error || "not listed"}
              </p>
            )}
