...
export function agentRoutePolicy(pathname: string, method: string): AgentRoutePolicy | null {
  ...
  if (pathname === "/auth/logout-all" && method === "POST") {
    return { permission: "users", write: true, ...WRITE };
  }
  if (pathname.startsWith("/auth/")) return { permission: "users", write: method !== "GET", ...WRITE };

  if (pathname === "/host" || pathname === "/host/status" || pathname === "/host/firewall") {
    return { permission: "dashboard", write: false };
  }
  if (pathname.startsWith("/game/")) {
    return { permission: "game", write: method !== "GET", ...WRITE };
  }
  if (pathname.startsWith("/missions")) {
    return { permission: "missions", write: method !== "GET", ...WRITE };
  }
  if (pathname.startsWith("/settings") || pathname.startsWith("/audit")) {
    return { permission: "settings", write: method !== "GET", ...WRITE };
  }
  ...
  if (instanceMatch) {
    ...
    return { permission: "instances", write: method !== "GET", ...WRITE };
  }

  return { permission: "dashboard", write: method !== "GET", ...WRITE };
}
